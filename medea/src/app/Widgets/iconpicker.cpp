...
void IconPicker::themeChanged()
{
    ...
    setStyleSheet("IconPicker {background-color: " % theme->getBackgroundColorHex() + ";border:1px solid " % theme->getDisabledBackgroundColorHex() % ";}" +
...
void IconPicker::setCurrentIcon(const QString& icon_prefix, const QString& icon_name)
{
    if (setup_layout) {
...
void IconPicker::setupLayout()
{
	...
	auto apply_toolbar = new QToolBar(this);
    ...
    for (const auto& icon_prefix : prefix_list) {
