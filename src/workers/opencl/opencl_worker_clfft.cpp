...
bool OpenCL_Worker::FFT(std::vector<float>& data)
{
    ...
    Buffer<float> buffer = manager_->CreateBuffer(data, *dev, true);

    auto success = FFT(buffer, allocated_dev_index, work_id);

    /* Fetch results of calculations. */
    try {
        data = buffer.ReadData(*dev, true);
...
bool OpenCL_Worker::FFT(Buffer<float>& buffer, int device_id, int work_id)
{
    ...
    auto& dev = manager_->GetDevices()[device_id];
    ...
    if(N > 1) {
        Log(GET_FUNC, Logger::WorkloadEvent::ERROR, work_id,
            "Unable to perform FFT: Number of samples must be a mix of powers of 2, 3 and 5");
        return false;
    }
    N = length;
    ...
    /* Execute the plan. */
    err = clfftEnqueueTransform(planHandle, CLFFT_FORWARD, 1, &dev_queue, 0, NULL, NULL, &(buffer.GetBackingRef()()),
                                NULL, NULL);
