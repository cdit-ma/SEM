...
private:
    ...
    //typename std::enable_if<!std::is_base_of<OpenCLBufferBase, T0>::value>::type
...
bool OpenCLKernel::SetArgsRecursive(unsigned int index, T0& arg0) {
    static_assert(!std::is_pointer<T0>::value, "SetArgs does not accept pointered types");
    //static_assert(!std::is_base_of<OpenCLBufferBase, T0>::value, "OpenCLBufferBase in the generic arg setter!");
    //std::cerr << "entering base call for type " << typeid(T0).name() <<std::endl;

    cl_int err;

    // If we are passed something derived from OpenCLBufferBase we know the backing reference will be properly handled
    if(std::is_base_of<OpenCLBufferBase, T0>::value) {
        const cl::Buffer& buffer_arg = reinterpret_cast<const OpenCLBufferBase*>(&arg0)->GetBackingRef();
