...
void LoggerPrinter::LogWorkerEvent(const Worker& worker, const std::string& function_name, const Logger::WorkloadEvent& event, int work_id, std::string args, bool print){
    if(print){
        std::lock_guard<std::mutex> lock(mutex_);
...
void LoggerPrinter::LogPortExceptionEvent(const Port& port, const ::BaseMessage& message, const std::string& error_string, bool print){
    if(print){
        std::lock_guard<std::mutex> lock(mutex_);
        auto c_ptr = port.get_component().lock();
        if(c_ptr){
...
void LoggerPrinter::LogPortExceptionEvent(const Port& port, const std::string& error_string, bool print){
    if(print){
        std::lock_guard<std::mutex> lock(mutex_);
        auto c_ptr = port.get_component().lock();
        if(c_ptr){
