	  ...
	  slider_(slider)
{
	...
	if (orientation_ == Qt::Horizontal) {
		...
		if (valueType_ == VALUE_TYPE::DATE_TIME) {
			...
			setMinimumWidth(fontMetrics().horizontalAdvance(QDateTime::fromMSecsSinceEpoch(0).toString(TIME_FORMAT)));
...
void AxisDisplay::setDisplayFormat(TIME_DISPLAY_FORMAT format)
{
	switch (format) {
		case TIME_DISPLAY_FORMAT::DATE_TIME:
			setMinimumHeight(ceil(minimumHeight() * 2.0));
			break;
		case TIME_DISPLAY_FORMAT::ELAPSED_TIME:
			setMinimumHeight(ceil(minimumHeight() / 2.0));
			break;
		default:
			// We are currently not using the VALUE display format
			return;
	}
	axisFormat_ = format;
	update();
...
void AxisDisplay::paintEvent(QPaintEvent* event)
{
	...
	if (displayHoverValue_) {
		QString hoveredStr = getConvertedString(hoveredValue_);
...
void AxisDisplay::paintHorizontal(QPainter &painter, QVector<QLineF> &tickLines, QRectF &rect)
{
	...
	for (int i = 0; i <= tickCount_; i++) {
		...
		if (tickCount_ == 1) {
			...
			tickX = penWidth_ / 2.0;
			value = displayedMin_;
			minTextWidth = fontMetrics().horizontalAdvance(getConvertedString(value));
			...
			tickX -= penWidth_ / 2.0;
			value = displayedMax_;
			maxTextWidth = fontMetrics().horizontalAdvance(getConvertedString(value));
			textRect = textRect.adjusted(0, 0, -maxTextWidth - penWidth_, 0);
		}

		tickLines.append(QLineF(tickX, tickY, tickX, tickY + lineLength));
		painter.drawText(textRect, getConvertedString(value), QTextOption(textAlignment_));
		startPoint += QPointF(rectWidth, 0);
		...
		if (dateTimeFormat) {
			...
			if (tickCount_ != 1) {
				if (i == 0) {
					auto strLengthDiff = fontMetrics().horizontalAdvance(dateStr) - minTextWidth;
					if (strLengthDiff > 0) {
						textRect.moveRight(textRect.right() + strLengthDiff - 5);
					}
				} else if (i == tickCount_) {
					auto strLengthDiff = fontMetrics().horizontalAdvance(dateStr) - maxTextWidth;
					if (strLengthDiff > 0) {
...
void AxisDisplay::rangeChanged()
{
	...
	if (orientation_ == Qt::Vertical) {
		...
		for (int i = 0; i <= tickCount_; i++) {
			...
			maxLength = qMax(maxLength, getConvertedString(value).length());
			tickY += rectHeight;
		}

		if (maxLength != widestTextLength_) {
			setFixedWidth(maxLength * fontMetrics().horizontalAdvance("0") + tickLength_ + spacing_ * 2);
...
QString AxisDisplay::getDateTimeString(double value)
{
	...
	auto maxElapsedMS = displayedMax_ - displayedMin_;
	auto maxElapsedDays = maxElapsedMS / 8.64e7;
	auto maxElapsedHours = maxElapsedMS / 3.6e6;
	auto maxElapsedMins = maxElapsedMS / 6e4;
...
QString AxisDisplay::getElapsedTimeString(double value)
{
	...
	auto maxElapsedMS = displayedMax_ - min_;
	auto maxElapsedDays = maxElapsedMS / 8.64e7;
	auto maxElapsedHours = maxElapsedMS / 3.6e6;
	auto maxElapsedMins = maxElapsedMS / 6e4;
...
/**
 * @brief AxisDisplay::getConvertedString
 * @param value
 * @return
 */
QString AxisDisplay::getConvertedString(double value)
{
	switch (axisFormat_) {
		case TIME_DISPLAY_FORMAT::ELAPSED_TIME:
...
{
	int halfPenWidth = ceil(penWidth_ / 2.0);
