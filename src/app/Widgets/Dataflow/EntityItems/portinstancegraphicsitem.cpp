...
void PortInstanceGraphicsItem::playEvents(qint64 from_time, qint64 to_time)
{
    ...
    /*if (!port_lifecycle_events.isEmpty() || !port_events.isEmpty()) {
        qDebug() << "Play events for between " << QDateTime::fromMSecsSinceEpoch(from_time).toString("hh:mm:ss.zzz") << " and "
                 << QDateTime::fromMSecsSinceEpoch(to_time).toString("hh:mm:ss.zzz");
        qDebug() << "Flash port " << getPortName() << getGraphmlID() << " - " << port_inst_data_.getPath();
        flashPort(from_time);
        for (const auto& e : port_events) {
            auto p_e = (PortEvent*) e;
            qDebug() << "event: " <<p_e << "," << e << "-" << p_e->getTypeString(p_e->getType());
            //qDebug() << p_e->toString("hh:mm:ss.zzz");
        }
        if (event_src_port_ && !port_events.isEmpty()) {
            qDebug() << "Flash edge";
            emit flashEdge(from_time, flash_duration_ms);
        }
        qDebug() << "--------------------------------------------------------";
