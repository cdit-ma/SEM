...
void Theme::setActiveWidgetBorderColor(const QColor& color)
{
...
void Theme::StoreActionIcon(QAction* action, const QString& icon_path, const QString& icon_alias)
{
...
void Theme::StoreActionIcon(QAction* action, const IconPair& icon)
{
...
void Theme::setDisabledBackgroundColor(const QColor& color)
{
...
void Theme::setAltBackgroundColor(const QColor& color)
{
...
void Theme::setHighlightColor(const QColor& color)
{
...
void Theme::setTextColor(ColorRole role, const QColor& color)
{
...
void Theme::setSeverityColor(Notification::Severity severity, const QColor& color)
{
...
void Theme::setMenuIconColor(ColorRole role, const QColor& color)
{
...
void Theme::setAspectBackgroundColor(VIEW_ASPECT aspect, const QColor& color)
{
...
bool Theme::gotImage(const IconPair& icon)
{
...
QIcon Theme::getIcon(const IconPair& icon)
{
...
{
    return _getPixmap(getResourceName(prefix, alias), size, std::move(tintColor));
}
...
/**
 * @brief Theme::getScrollAreaStyleSheet
 * @return
 */
QString Theme::getScrollAreaStyleSheet()
{
    return "QScrollArea {"
...
QString Theme::getWidgetStyleSheet(const QString& widgetName)
{
    return widgetName % " {"
           "background:" % getBackgroundColorHex() % ";"
           "color:" % getTextColorHex() % ";"
...
QString Theme::getTabbedWidgetStyleSheet()
{
    return "QTabBar::tab{ background:" % getAltBackgroundColorHex() % "; color: " % getTextColorHex() % "; border-radius:" % getCornerRadius() % "; border: 1px solid " % getBackgroundColorHex() % ";}"
           ...
           "QTabBar::tab:left{ border-top-right-radius: 0px; border-bottom-right-radius: 0px; margin: 0px 1px; padding: 10px 5px; }"
           "QTabBar::tab:right{ border-top-left-radius: 0px; border-bottom-left-radius: 0px; margin: 0px 1px; padding: 10px 5px; }"
...
QString Theme::getDockTitleBarStyleSheet(bool isActive, const QString& widgetName)
{
...
QString Theme::getLineEditStyleSheet(const QString& widget_name)
{
    return widget_name + " {"
           "margin: 0px;"
           "padding: 0px;"
...
{   
    ...
    entity_icons_load_future = QtConcurrent::run([=]() {
        ...
        qInfo() << "Preloaded EntityIcons #" << results.count() << " Images in: " <<  time_finish - time_start << "MS";
    });
    ...
    auto time_finish = QDateTime::currentDateTime().toMSecsSinceEpoch();
    qInfo() << "Preloaded #" << preload_Icons.count() << " Images in: " <<  time_finish - time_start << "MS";
    emit theme_Changed();
}

void Theme::settingChanged(SETTINGS setting, const QVariant& value)
{
...
QString resourceName(const QString& prefix, const QString& alias)
{
    return "Images/" % prefix % '/' % alias;
}

QString Theme::getResourceName(const QString& prefix, const QString& alias) const
{
...
QString Theme::getResourceName(const IconPair& icon) const
{
...
QStringList Theme::getIcons(const QString& icon_prefix)
{
    ...
    if (icon_prefix_lookup.contains(icon_prefix)) {
        icons = icon_prefix_lookup[icon_prefix].values();
    }
    return icons;
}