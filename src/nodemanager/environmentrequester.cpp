...
void EnvironmentRequester::HeartbeatLoop(){
    ...
    try{
        update_socket = new zmq::socket_t(*context_, ZMQ_REQ);
    ...
    //TODO: Contidtion var to break out
    while(true){
        if(request_queue_.empty()){
            ...
            if(trigger == std::cv_status::timeout){
                ZMQSendTwoPartRequest(update_socket, "HEARTBEAT", "");
                auto reply = ZMQReceiveTwoPartReply(update_socket);
            }
        }

        else{
            //Pull update off queue and send
            auto request = request_queue_.front();
            request_queue_.pop();

            ZMQSendTwoPartRequest(update_socket, request.request_type_, request.request_data_);
            auto reply = ZMQReceiveTwoPartReply(update_socket);
...
int EnvironmentRequester::GetPort(const std::string& component_id, const std::string& component_info){
    int port = 0;
    auto port_response = SendRequest("ASSIGNMENT_REQUEST", component_id);
...
void EnvironmentRequester::ZMQSendTwoPartRequest(zmq::socket_t* socket, const std::string& request_type, 
    ...
    try{
        ...
        std::cout << error.what() << "in EnvironmentRequester::SendTwoPartRequest" << std::endl;
    }
}
