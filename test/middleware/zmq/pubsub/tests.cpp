...
class zmq_SubPort_FSMTester : public ActivatableFSMTester{
    protected:
        void SetUp(){
            ...
            EXPECT_TRUE(setup_sub_port(*port, 7000));
            a = port;
            ASSERT_TRUE(a);
        }
};
...
protected:
    void SetUp(){
        ...
        EXPECT_TRUE(setup_pub_port(*port, 7000));
        a = port;
        ASSERT_TRUE(a);
    }
};
...
TEST(zmq_PubSub, Basic_Stable){
    ...
    //Setup Ports
    EXPECT_TRUE(setup_pub_port(pub_port, 7000));
    EXPECT_TRUE(setup_sub_port(sub_port, 7000));
