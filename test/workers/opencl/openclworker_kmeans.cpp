...
std::ostream& operator<<(std::ostream& os, const KMeansParam& kmp) {
    return os << kmp.device << "points: [" << print_vec(kmp.points)
                            << "] - centroids: [" << print_vec(kmp.centroids)
...
struct KMeansData {
    KMeansData(const std::vector<float>& points, const std::vector<float>& centers, const std::vector<int>& classifications) {
        this->points = points;
...
std::vector<KMeansParam> getKMeansParamPermutations(
...
INSTANTIATE_TEST_CASE_P(DISABLED_KMeans, KMeansFixture, ::testing::ValuesIn(getKMeansParams()));
