<xsl:stylesheet version="2.0"
    ...
    <xsl:function name="o:nl">
        ...
        <xsl:variable name="total">
             <xsl:for-each select="1 to $count"><xsl:text>&#xa;</xsl:text></xsl:for-each>
        </xsl:variable>
    ...
    <xsl:function name="o:lib_include">
        ...
        <xsl:value-of select="concat('#include ', o:angle_wrap($lib), o:nl(1))" />
    </xsl:function>	

    <xsl:function name="o:local_include">
        <xsl:param name="lib" as="xs:string"  />
        <xsl:value-of select="concat('#include ', o:dblquote_wrap($lib), o:nl(1))" />
    </xsl:function>
    ...
    <xsl:function name="o:cmake_find_library">
        ...
        <xsl:value-of select="concat('find_library(', $lib_vars, ' ', $lib, ' ', o:dblquote_wrap($lib_path), ')', o:nl(1))" />
    </xsl:function>
    ...
    <xsl:function name="o:cmake_find_package">
        ...
        <xsl:value-of select="concat('find_package(', $lib, if($args != '') then ' ' else '', $args, ')', o:nl(1))" />
    </xsl:function>

    <xsl:function name="o:cmake_set_env">
        <xsl:param name="var" as="xs:string"  />
        <xsl:param name="val" as="xs:string"  />
        <xsl:value-of select="concat('set(', $var, ' ',  o:dblquote_wrap($val), ')', o:nl(1))" />
    </xsl:function>
    ...
    <xsl:function name="o:cmake_include_dir">
        ...
        <xsl:value-of select="concat('include_directories(', o:dblquote_wrap($dir), ')', o:nl(1))" />
    </xsl:function>

    <xsl:function name="o:cmake_comment">
        <xsl:param name="text" as="xs:string"  />
        <xsl:value-of select="concat('# ', $text, o:nl(1))" />
    </xsl:function>

    <xsl:function name="o:define_guard">
        <xsl:param name="lib_name" as="xs:string"  />
        
        <xsl:value-of select="concat('#ifndef ', $lib_name, o:nl(1))" />
        <xsl:value-of select="concat('#define ', $lib_name, o:nl(1))" />
        <xsl:value-of select="o:nl(1)" />
    </xsl:function>

    <xsl:function name="o:define_guard_end">
        <xsl:param name="lib_name" as="xs:string" />
        <xsl:value-of select="concat(o:nl(1), '#endif //', $lib_name, o:nl(1))" />
    </xsl:function>
    ...
    <xsl:function name="o:process_member">
        ...
        <xsl:choose>
             <xsl:when test="o:member_requires_cast($member_type, $dst_mw) = true()">
                ...
                <!-- Protobuf uses $var_name($index) -->
                <xsl:value-of select="concat(o:t(1), $temp_type, ' ', $temp_var, ' = ', $value, ';', o:nl(1))" />
                <xsl:value-of select="concat(o:t(1), $out_var, o:fp(), o:cpp_mw_set_func($member_label, $dst_mw, $temp_var), ';', o:nl(1))" />
            </xsl:when>
            <xsl:otherwise>
                <!-- Set the value; using the appropriate middlewares setter -->
                <xsl:value-of select="concat(o:t(1), $out_var, o:fp(), o:cpp_mw_set_func($member_label, $dst_mw, $value), ';', o:nl(1))" />
            </xsl:otherwise>
        </xsl:choose>
    </xsl:function>
    ...
    <xsl:function name="o:get_convert_h">
        ...
        <xsl:value-of select="concat(o:t(1), $mw_type, '* translate(const ', $base_type , o:and(), ' val);', o:nl(1))" />
        <xsl:value-of select="concat(o:t(1), $base_type, '* translate(const ', $mw_type , o:and(), ' val);', o:nl(1))" />

        <xsl:if test="lower-case($mw) = 'proto'">
            <!-- Helper functions -->
                <xsl:value-of select="o:nl(1)" />
                <xsl:value-of select="o:tabbed_cpp_comment('Helper Functions', 1)" />
                <xsl:value-of select="concat(o:t(1), 'template ', o:angle_wrap('class T'), ' ', $base_type, '* decode(const std::string', o:and(), ' val);', o:nl(1))" />
                <xsl:value-of select="concat(o:t(1), 'std::string ', 'encode(const ', $base_type, o:and(), ' val);', o:nl(1))" />
                ...
                <xsl:value-of select="concat(o:t(1), 'template ', o:angle_wrap(''), ' ', $base_type, '* decode', o:angle_wrap($mw_type), '(const std::string', o:and(), ' val);', o:nl(1))" />
        </xsl:if>
    ...
    <xsl:function name="o:get_convert_cpp">
        ...
        <xsl:if test="lower-case($mw) = 'proto'">
            ...
            <xsl:value-of select="concat('template', o:angle_wrap(''), o:nl(1))" />
            <xsl:value-of select="concat($base_type, '* ', $mw, '::decode', o:angle_wrap($mw_type), '(const std::string', o:and(), ' val){', o:nl(1))" />
            <xsl:value-of select="concat(o:t(1), $mw_type, ' out_;', o:nl(1))" />
            <xsl:value-of select="concat(o:t(1), 'out_.ParseFromString(val);', o:nl(1))" />
            <xsl:value-of select="concat(o:t(1), 'return translate(out_);', o:nl(1))" />
            <xsl:value-of select="concat('};', o:nl(1))" />
            <xsl:value-of select="o:nl(1)" />

            <!-- encode function base->str -->
            <xsl:value-of select="concat('std::string proto::encode(const ', $base_type, o:and(), ' val){', o:nl(1))" />
            <xsl:value-of select="concat(o:t(1), 'std::string out_;', o:nl(1))" />
            <xsl:value-of select="concat(o:t(1), 'auto pb_ = translate(val);', o:nl(1))" />
            <xsl:value-of select="concat(o:t(1), 'pb_', o:fp(), 'SerializeToString(', o:and(), 'out_);', o:nl(1))" />
            <xsl:value-of select="concat(o:t(1), 'delete pb_;', o:nl(1))" />
            <xsl:value-of select="concat(o:t(1), 'return out_;', o:nl(1))" />
            <xsl:value-of select="concat('};', o:nl(1))" />
            <xsl:value-of select="o:nl(1)" />
        </xsl:if>
    </xsl:function>
    ...
    <xsl:function name="o:cmake_add_subdirectory">
        <xsl:param name="dir" />
        <!-- set RE_PATH -->
        <xsl:value-of select="concat('add_subdirectory(', o:dblquote_wrap($dir), ')',o:nl(1))" />
    </xsl:function>
    ...
    <xsl:function name="o:cmake_find_mw_package">
        ...
        <xsl:if test="$package != ''">
            ...
            <xsl:value-of select="concat('if(NOT ', upper-case($package), '_FOUND)', o:nl(1))" />

            <xsl:value-of select="concat(o:t(1),'message(STATUS ', o:dblquote_wrap(concat('Cannot find ', $package, ' cannot build project ', o:cmake_var_wrap('PROJ_NAME'))), ')', o:nl(1))" />
            <xsl:value-of select="concat(o:t(1), 'return()', o:nl(1))" />
            <xsl:value-of select="concat('endif()', o:nl(1))" />
            <xsl:value-of select="o:nl(1)" />
        </xsl:if>
    </xsl:function>
    ...
    <xsl:function name="o:cmake_generate_mw_files">
        ...
        <xsl:value-of select="concat($mw_gen,'(', $srcs, ' ', $hdrs, ' ', $aggregate_name_lc, '.', $mw_ext, ')', o:nl(1))" />
    </xsl:function>
    ...
    <xsl:function name="o:get_base_data_type_h">
        ...
        <xsl:value-of select="concat('namespace Base{', o:nl(1))" />

        <!-- Define the class, Inherits from BaseMessage -->
        <xsl:if test="$namespace != ''">
            <xsl:value-of select="concat(o:t(1), 'namespace ', $namespace, '{', o:nl(1))" />
        </xsl:if>
        <xsl:value-of select="concat(o:t(2), 'class ', $aggregate_label_cc, ' : public ::BaseMessage{', o:nl(1))" />
        <!-- Handle Members -->
        ...
        <xsl:for-each select="$aggregate_insts">
            ...
            <xsl:value-of select="concat(o:nl(1), o:t(2))" />
    ...
    <xsl:function name="o:get_base_enum_h">
       ...
       <!-- Define Guard -->
        ...
        <xsl:if test="$enum_namespace != ''">
            <xsl:value-of select="concat(o:t(1), 'namespace ', $enum_namespace, '{', o:nl(1))" />
        </xsl:if>

        <!-- define the enum class -->
        <xsl:value-of select="concat(o:t($tab_offset), ' enum class ', $enum_label_cc, '{', o:nl(1))" />
        ...
        <xsl:for-each select="$enum_members">
            ...
            <xsl:value-of select="concat(o:t($tab_offset + 1), $member_label_uc, if($is_last) then '' else ',' , o:nl(1))" />
        </xsl:for-each>

        <xsl:value-of select="concat(o:t($tab_offset), '};', o:nl(1))" />
        
        <!--  Define a toString
        <xsl:value-of select="o:nl(1)" />
        <xsl:value-of select="o:tabbed_cpp_comment(concat('enum ', $enum_label ,'::ToString()'), $tab_offset)" />
        <xsl:value-of select="concat(o:t($tab_offset), 'std::string ToString(', $enum_label_cc, ' e){', o:nl(1))" />
        <xsl:value-of select="concat(o:t($tab_offset + 1), 'switch(e){', o:nl(1))" />
        
        <xsl:for-each select="$enum_members">
            <xsl:variable name="is_last" select="position() = last()" />
            <xsl:variable name="member_label" select="cdit:get_key_value(., 'label')" />
            <xsl:variable name="member_label_uc" select="upper-case($member_label)" />
            <xsl:value-of select="concat(o:t($tab_offset + 2),'case ', $enum_label_cc, '::', $member_label_uc,':', o:nl(1))" />
            <xsl:value-of select="concat(o:t($tab_offset + 3),'return ', o:dblquote_wrap($member_label_uc), ';', o:nl(1))" />
        </xsl:for-each>
        <xsl:value-of select="concat(o:t($tab_offset + 1), '};', o:nl(1))" />
        <xsl:value-of select="concat(o:t($tab_offset), '};', o:nl(1))" />
        -->
    ...
    <xsl:function name="o:get_base_enum_cmake">
        ...
        <xsl:value-of select="concat(o:t(1), '${CMAKE_CURRENT_SOURCE_DIR}/', $enum_label_lc, '.h', o:nl(1))" />
        <xsl:value-of select="concat(o:t(1), ')', o:nl(1))" />
        <xsl:value-of select="o:nl(1)" />

        <xsl:value-of select="concat('if (MSVC)', o:nl(1))" />
        <xsl:value-of select="concat(o:t(1), 'set(CMAKE_WINDOWS_EXPORT_ALL_SYMBOLS ON)', o:nl(1))" />
        <xsl:value-of select="concat('endif (MSVC)', o:nl(1), o:nl(1))" />

        <!-- add library/link -->
        <xsl:value-of select="concat('add_library(${PROJ_NAME} SHARED ${HEADERS})', o:nl(1))" />
    </xsl:function>
     ...
     <xsl:function name="o:get_base_data_type_cmake">
        ...
        <xsl:value-of select="concat(o:t(1), '${CMAKE_CURRENT_SOURCE_DIR}/', $aggregate_label_lc, '.cpp', o:nl(1))" />
        <xsl:value-of select="concat(o:t(1), ')', o:nl(1))" />
        <xsl:value-of select="o:nl(1)" />

        <!-- Set Headers files -->
        <xsl:value-of select="concat('set(HEADERS', o:nl(1))" />
        <xsl:value-of select="concat(o:t(1), '${CMAKE_CURRENT_SOURCE_DIR}/', $aggregate_label_lc, '.h', o:nl(1))" />
        <xsl:value-of select="concat(o:t(1), ')', o:nl(1))" />
        <xsl:value-of select="o:nl(1)" />
        ...
        <!-- add library/link -->
        <xsl:value-of select="concat('add_library(${PROJ_NAME} SHARED ${SOURCE} ${HEADERS})', o:nl(1))" />
    ...
    <xsl:function name="o:cmake_target_link_libraries">
        ...
        <xsl:value-of select="concat('target_link_libraries(', $proj_name, ' ', $library, ')', o:nl(1))" />
    </xsl:function>
    ...
    <xsl:function name="o:get_mw_type_cmake">
        ...
        <xsl:if test="$build_shared_library">
            ...
            <xsl:value-of select="concat('configure_file(', $aggregate_label_lc, '.', $mw_ext, ' ${CMAKE_CURRENT_BINARY_DIR} COPYONLY)', o:nl(1))" />
            <xsl:value-of select="o:nl(1)" />
            ...
            <xsl:if test="count($aggregate_definitions)">
                ...
                <xsl:for-each-group select="$aggregate_definitions" group-by=".">
                    ...
                    <xsl:choose>
                        <xsl:when test="$agg_ns = ''">
                            <xsl:value-of select="concat('configure_file(', $rel_path, $agg_la, '/', $agg_la, '.', $mw_ext, ' ${CMAKE_CURRENT_BINARY_DIR} COPYONLY)', o:nl(1))" />
                        </xsl:when>
                        <xsl:otherwise>
                            <xsl:value-of select="concat('configure_file(', $rel_path, $agg_ns, '/', $agg_la, '/', $agg_la, '.', $mw_ext, ' ${CMAKE_CURRENT_BINARY_DIR} COPYONLY)', o:nl(1))" />
                        </xsl:otherwise>
                    </xsl:choose>
                </xsl:for-each-group>
                <xsl:value-of select="o:nl(1)" />
            ...
            <!-- Set Source files -->
            <xsl:value-of select="concat('set(SOURCE', o:nl(1))" />
            <xsl:value-of select="concat(o:t(1), '${CMAKE_CURRENT_SOURCE_DIR}/convert.cpp', o:nl(1))" />
            <xsl:value-of select="concat(o:t(1), ')', o:nl(1))" />
            <xsl:value-of select="o:nl(1)" />

            <!-- Set HEADER files -->
            <xsl:value-of select="concat('set(HEADERS', o:nl(1))" />
            <xsl:value-of select="concat(o:t(1), '${CMAKE_CURRENT_SOURCE_DIR}/convert.h', o:nl(1))" />
            <xsl:value-of select="concat(o:t(1), ')', o:nl(1))" />
            <xsl:value-of select="o:nl(1)" />
        </xsl:if>
        ...
        <xsl:if test="$mw = 'proto'">
            <xsl:value-of select="concat(o:t(1), 'add_definitions(-DPROTOBUF_USE_DLLS)', o:nl(1))" />
        </xsl:if>
        <xsl:value-of select="concat(o:t(1), 'set(CMAKE_WINDOWS_EXPORT_ALL_SYMBOLS ON)', o:nl(1))" />
        <xsl:value-of select="concat('endif (MSVC)', o:nl(1), o:nl(1))" />
        ...
        <xsl:if test="$build_shared_library">
            ...
            <xsl:value-of select="concat('add_library(${SHARED_LIB_NAME} SHARED ${SOURCE} ${HEADERS} ', o:cmake_var_wrap($mw_srcs), ' ',o:cmake_var_wrap($mw_hdrs), ')', o:nl(1))" />
            <xsl:value-of select="o:nl(1)" />
            ...
            <xsl:value-of select="concat('target_link_libraries(${SHARED_LIB_NAME} ${RE_CORE_LIBRARIES})', o:nl(1))" />

            <!-- Link against the middleware libraries -->
            <xsl:if test="$mw_package_uc != ''">
                <xsl:value-of select="concat('target_link_libraries(${SHARED_LIB_NAME} ${', $mw_package_uc, '_LIBRARIES})', o:nl(1))" />
            </xsl:if>

            <!-- Link against the middleware helper libraries -->
            <xsl:if test="$build_module">
                <xsl:value-of select="concat('target_link_libraries(${SHARED_LIB_NAME} ${', $mw_helper_libs, '})', o:nl(1))" />
            </xsl:if>
            ...
            <xsl:value-of select="concat('target_link_libraries(${SHARED_LIB_NAME} ', $base_lib, ')', o:nl(1))" />
            <xsl:value-of select="o:nl(1)" />
            ...
            <xsl:if test="count($aggregate_definitions)">
                ...
                <xsl:for-each-group select="$aggregate_definitions" group-by=".">
                    ...
                    <xsl:value-of select="concat('target_link_libraries(${SHARED_LIB_NAME} ', $proto_lib, '_lib)', o:nl(1))" />
                </xsl:for-each-group>
                <xsl:value-of select="o:nl(1)" />
            </xsl:if>
        </xsl:if>
        ...
        <xsl:if test="$build_module">
            ...
            <xsl:value-of select="concat('add_library(${PROJ_NAME} MODULE ${CMAKE_CURRENT_SOURCE_DIR}/libportexports.cpp)', o:nl(1))" />
            <xsl:value-of select="o:nl(1)" />
            
            <xsl:value-of select="o:cmake_comment('Link the shared library.')" />
            <xsl:value-of select="concat('target_link_libraries(${PROJ_NAME} ${RE_CORE_LIBRARIES})', o:nl(1))" />
            <xsl:value-of select="concat('target_link_libraries(${PROJ_NAME} ${', $mw_helper_libs, '})', o:nl(1))" />

            <xsl:if test="$mw_package_uc != ''">
                <xsl:value-of select="concat('target_link_libraries(${PROJ_NAME} ${', $mw_package_uc, '_LIBRARIES})', o:nl(1))" />
            </xsl:if>

            <!-- Link against base datatype libraries -->
            <xsl:value-of select="o:cmake_comment('Link the shared library against the base type library')" />
            <xsl:variable name="base_lib" select="o:get_aggregate_lib_name($aggregate_root, 'base')" />
            <xsl:value-of select="concat('target_link_libraries(${PROJ_NAME} ', $base_lib ,')', o:nl(1))" />
            <xsl:value-of select="o:nl(1)" />
            ...
            <xsl:choose>
                <xsl:when test="$build_shared_library">
                    ...
                    <xsl:value-of select="concat('target_link_libraries(${PROJ_NAME} ${SHARED_LIB_NAME})', o:nl(1))" />
                    <xsl:value-of select="o:nl(1)" />
                </xsl:when>
                <xsl:when test="o:middleware_uses_protobuf($mw)">
                    <xsl:variable name="lib" select="o:get_aggregate_lib_name($aggregate_root, 'proto')" />
                    <xsl:value-of select="concat('target_link_libraries(${PROJ_NAME} ', $lib ,'_lib)', o:nl(1))" />
                </xsl:when>
            </xsl:choose>
        </xsl:if>
        ...
        <xsl:if test="$mw = 'rti'">
            ...
            <xsl:value-of select="concat('target_compile_definitions(${PROJ_NAME} PRIVATE -DRTI_UNIX -DRTI_64BIT)', o:nl(1))" />
            <xsl:value-of select="concat('target_compile_definitions(${SHARED_LIB_NAME} PRIVATE -DRTI_UNIX -DRTI_64BIT)', o:nl(1))" />
            <xsl:value-of select="o:nl(1)" />
        </xsl:if>
    </xsl:function>
    ...
    <xsl:function name="o:get_idl">
        ...
        <xsl:if test="count($enum_instances)">
            ...
            <xsl:for-each-group select="cdit:get_definitions($enum_instances)" group-by=".">
                ...
                <xsl:value-of select="concat('enum ', $enum_label, ' {', o:nl(1))" />
                    <xsl:for-each select="cdit:get_child_entities_of_kind(., 'EnumMember')">
                        ...
                        <xsl:value-of select="concat(o:t(1), $enum_member, if($is_last) then '' else ',',  o:nl(1))" />
                    </xsl:for-each>
                <xsl:value-of select="concat('};', o:nl(1))" />
            </xsl:for-each-group>
            <xsl:value-of select="o:nl(1)" />
        ...
        <xsl:if test="$aggregate_namespace != ''">
            <xsl:value-of select="concat('module ', $aggregate_namespace, '{', o:nl(1))" />
        </xsl:if>
            
        <!-- Struct Name -->
        <xsl:value-of select="concat(o:t($tab_stop), 'struct ', $aggregate_label_cc, '{', o:nl(1))" />
        ...
        <xsl:value-of select="concat(o:t($tab_stop),'};', o:nl(1))" />
        ...
        <xsl:for-each select="cdit:get_child_nodes($aggregate_root)">
            ...
            <xsl:variable name="is_key" select="cdit:is_key_value_true(., 'key')" />
            <xsl:if test="$is_key">
                <xsl:value-of select="concat(o:t($tab_stop),'#pragma keylist ', $aggregate_label_cc, ' ', $label, o:nl(1))" />
            </xsl:if>
        </xsl:for-each>
    ...
    <xsl:function name="o:get_proto">
        ...
        <!-- Using Protobuf 3 -->
        <xsl:value-of select="concat('syntax = ', o:dblquote_wrap('proto3'), ';', o:nl(1), o:nl(1))" />
        ...
        <xsl:if test="count($aggregate_instances)">
            ...
            <xsl:for-each-group select="cdit:get_definitions($aggregate_instances)" group-by=".">
                <xsl:variable name="aggregate_label" select="lower-case(cdit:get_key_value(., 'label'))" />
                <xsl:value-of select="concat('import ', o:dblquote_wrap(concat($aggregate_label, '.proto')), ';', o:nl(1))" />
            </xsl:for-each-group>
            <xsl:value-of select="o:nl(1)" />
        </xsl:if>
        ...
        <xsl:if test="count($enum_instances)">
            ...
            <xsl:for-each-group select="cdit:get_definitions($enum_instances)" group-by=".">
                ...
                <xsl:value-of select="concat('enum ', $enum_label, ' {', o:nl(1))" />
                    <xsl:for-each select="cdit:get_child_entities_of_kind(., 'EnumMember')">
                        <xsl:variable name="enum_member" select="upper-case(cdit:get_key_value(., 'label'))" />
                        <xsl:value-of select="concat(o:t(1), $enum_member, ' = ', position() - 1, ';',  o:nl(1))" />
                    </xsl:for-each>
                <xsl:value-of select="concat('}', o:nl(1))" />
            </xsl:for-each-group>
            <xsl:value-of select="o:nl(1)" />
        ...
        <xsl:if test="$aggregate_namespace != ''">
            <xsl:value-of select="concat('package ', $aggregate_namespace, ';', o:nl(1))" />
        </xsl:if>

        <xsl:value-of select="concat('message ', $aggregate_label_cc, '{', o:nl(1))" />
        ...
        <xsl:for-each select="cdit:get_child_nodes($aggregate_root)">
            ...
            <xsl:choose>    
                <xsl:when test="$kind = 'Member'">
                    ...
                    <xsl:value-of select="concat(o:t(1), $proto_type, ' ', $label, ' = ', $index, ';', o:nl(1))" />
                </xsl:when>
                <xsl:when test="$kind = 'Vector'">
                    <xsl:variable name="is_vector_complex" select="cdit:is_vector_complex(.)" />
                    ...
                    <xsl:value-of select="concat(o:t(1), 'repeated ', $proto_type, ' ', $label, ' = ', $index, ';', o:nl(1))" />
                </xsl:when>
                <xsl:when test="$kind = 'AggregateInstance'">
                    ...
                    <xsl:value-of select="concat(o:t(1), $aggregate_obj, ' ', $label, ' = ', $index, ';', o:nl(1))" />
                </xsl:when>
                <xsl:when test="$kind = 'EnumInstance'">
                    <xsl:variable name="enum_definition" select="cdit:get_definition(.)" />
                    <xsl:variable name="enum_type" select="o:camel_case(cdit:get_key_value($enum_definition, 'label'))" />
                    <xsl:value-of select="concat(o:t(1), $enum_type, ' ', $label, ' = ', $index, ';', o:nl(1))" />
                </xsl:when>
                <xsl:otherwise>
    ...
    <xsl:function name="o:process_aggregate">
        ...
        <xsl:value-of select="concat(o:t(1), 'auto ', $src_var, ' = ', $get_value, ';', o:nl(1))" />
        <xsl:value-of select="concat(o:t(1), 'auto ', $dst_var, ' = ', $class_namespace, '::translate(', $src_var, ');', o:nl(1))" />
                
        <xsl:value-of select="concat(o:t(1), 'if(', $dst_var, '){', o:nl(1))" />
            <xsl:choose>
                <xsl:when test="o:middleware_uses_protobuf($dst_mw)">
                    <!-- Protobuf have to use a swap function from the newly added element -->
                    <xsl:value-of select="concat(o:t(2), $out_var, o:fp(), 'set_allocated_', o:cpp_mw_func($aggregate_label, $dst_mw), '(', $dst_var, ');', o:nl(1))" />
                </xsl:when>
                <xsl:otherwise>
                    <!-- Complex vectors in other middlewares use indexed insertion using a dereferenced pointer -->
                    <xsl:value-of select="concat(o:t(2), $out_var, o:fp(), o:cpp_mw_set_func($aggregate_label, $dst_mw, concat('*', $dst_var)), ';', o:nl(1))" />
                </xsl:otherwise>
            </xsl:choose>
            <!-- Free Memory From Translate -->
            <!--<xsl:value-of select="concat(o:t(2), 'delete ', $dst_var, ';', o:nl(1))" />-->
            <xsl:value-of select="concat(o:t(1), '}', o:nl(1))" />
    </xsl:function>
    ...
    <xsl:function name="o:process_enum">
        ...
        <xsl:value-of select="concat(o:t(1), $out_var, o:fp(), o:cpp_mw_set_func($enum_label, $dst_mw, $static_cast_value), ';', o:nl(1))" />
    </xsl:function>
    ...
    <xsl:function name="o:get_translate_cpp">
        ...
        <xsl:value-of select="concat($dst_type, '* ', $class_namespace, '::translate(const ', $src_type , o:and(), ' ', $in_var, '){', o:nl(1))" />
        <xsl:value-of select="concat(o:t(1), 'auto ', $out_var, ' = new ', $dst_type, '();', o:nl(1))" />
        ...
        <xsl:for-each select="cdit:get_child_nodes($aggregate_root)">
            ...
            <xsl:choose>    
                ...
                <xsl:otherwise>
                    <xsl:value-of select="concat(o:t($tab_stop + 1), '// Kind ', $kind, ' Not implemented!', o:nl(1))" />
                </xsl:otherwise>
            </xsl:choose>
        </xsl:for-each>

         <!-- Return the Translated object -->
        <xsl:value-of select="concat(o:t(1), 'return ', $out_var, ';', o:nl(1))" />
        <xsl:value-of select="concat('};', o:nl(1))" />
        <xsl:value-of select="o:nl(1)" />
    </xsl:function>
    ...
    <xsl:function name="o:process_vector">
        ...
        <xsl:value-of select="concat(o:t(2), 'auto ', $vector_copy, ' = ', $in_var, o:fp(), o:cpp_mw_get_func($vector_label, $src_mw), ';', o:nl(1))" />

        <!-- Get the source vector size -->
        <xsl:value-of select="o:tabbed_cpp_comment('Get the size of the source vector', 2)" />
        <xsl:value-of select="concat(o:t(2), 'auto size_ = ', $vector_copy, '.size();', o:nl(1))" />
        ...
        <xsl:if test="$dst_mw_uses_pb = false()">
            ...
            <xsl:value-of select="concat(o:t(2), $out_var, o:fp(), o:cpp_mw_func($vector_label, $dst_mw), '().resize(size_);', o:nl(1))" />
        </xsl:if>

        <!-- Define for loop -->
        <xsl:value-of select="o:nl(1)" />
        <xsl:value-of select="o:tabbed_cpp_comment('Itterate through source vector', 2)" />
        <xsl:value-of select="concat(o:t(2), 'for(int i = 0; i ', o:lt(), ' size_; i++){', o:nl(1))" />
        ...
        <xsl:choose>
            <xsl:when test="$is_vector_complex">
                ...
                <xsl:value-of select="concat(o:t(3), 'auto ', $dst_var, ' = ', $class_namespace, '::translate(',o:and(),  o:bracket_wrap($get_src_val), ');', o:nl(1))" />
                
                <xsl:value-of select="concat(o:t(3), 'if(', $dst_var, '){', o:nl(1))" />
                <xsl:choose>
                    <xsl:when test="o:middleware_uses_protobuf($dst_mw)">
                        ...
                        <xsl:value-of select="concat(o:t(4), 'auto ', $new_element, ' = ', $out_var, o:fp(), o:cpp_mw_func(concat('add_', $vector_label), $dst_mw),'();', o:nl(1))" />
                        ...
                        <xsl:value-of select="concat(o:t(4), $new_element, o:fp(), 'Swap(', $dst_var, ');', o:nl(1))" />
                    </xsl:when>
                    <xsl:otherwise>
                        <!-- Complex vectors in other middlewares use indexed insertion using a dereferenced pointer -->
                        <xsl:value-of select="concat(o:t(4), $out_var, o:fp(), o:cpp_mw_func($vector_label, $dst_mw), '()[i] = *', $dst_var, ';', o:nl(1))" />
                    </xsl:otherwise>
                </xsl:choose>
                <!-- Free Memory From Translate -->
                <xsl:value-of select="concat(o:t(4), 'delete ', $dst_var, ';', o:nl(1))" />
                <xsl:value-of select="concat(o:t(3), '}', o:nl(1))" />
            </xsl:when>
            <xsl:otherwise>
                <xsl:choose>
                    <xsl:when test="o:middleware_uses_protobuf($dst_mw)">
                        <!-- Primitive vectors in protobuf use an add_ function with value as a parameter -->
                        <xsl:value-of select="concat(o:t(3), $out_var, o:fp(), o:cpp_mw_func(concat('add_', lower-case($vector_label)), $src_mw),'(', $get_src_val, ');', o:nl(1))" />
                    </xsl:when>
                    <xsl:otherwise>
                        <!-- Primitive vectors in other middlewares simply use indexed insertion -->
                        <xsl:value-of select="concat(o:t(3), $out_var, o:fp(), o:cpp_mw_func($vector_label, $dst_mw), '()[i] = ', $get_src_val, ';', o:nl(1))" />
                    </xsl:otherwise>
                </xsl:choose>
            </xsl:otherwise>
        </xsl:choose>
    ...
    <xsl:function name="o:cpp_set_translated_val">
        ...
        <xsl:value-of select="concat(o:t($tab), '{', o:nl(1))" />
        <xsl:value-of select="concat(o:t($tab+1), 'auto ', $mw_var_name, ' = ', $mw, '::translate(', o:and(), $var_name ,');', o:nl(1))" />
        <xsl:value-of select="concat(o:t($tab+1), 'if(', $mw_var_name, '){', o:nl(1))" />
        <xsl:value-of select="concat(o:t($tab+2), 'out_', o:fp(), o:cpp_mw_get_func($var_name, $mw), '()[i] = ', '*', $mw_var_name, ';', o:nl(1))" />
        <xsl:value-of select="concat(o:t($tab+2), 'delete ', $mw_var_name, ';', o:nl(1))" />
        <xsl:value-of select="concat(o:t($tab+1), '}', o:nl(1))" />
        <xsl:value-of select="concat(o:t($tab), '}', o:nl(1))" />
    ...
    <xsl:function name="o:construct_port_constructor">
        ...
        <xsl:value-of select="concat('EventPort* ', $fn_name, '(const std::string', o:and(), ' port_name, ', o:cpp_weak_ptr('Component'), ' component){', o:nl(1))" />
        <xsl:value-of select="concat(o:t(1), 'return ', $fn_name, $mw_class, o:bracket_wrap('port_name, component'), ';', o:nl(1))" />
        <xsl:value-of select="concat('};', o:nl(1))" />
    </xsl:function>
   ...
   <xsl:function name="cdit:middlewares_match" as="xs:boolean">
    ...
    <xsl:function name="o:namespace">
        <xsl:param name="namespace" as="xs:string" />
        <xsl:value-of select="concat('namespace ', $namespace, '{', o:nl(1))" />
    </xsl:function>
    ...
    <xsl:function name="o:declare_attribute_functions">
        ...
        <!-- Copy Setter -->
        <xsl:value-of select="concat(o:t(2), o:cpp_func_def('void', '', o:cpp_base_set_func($variable_name), concat('const ', $variable_type, o:and(), ' val')), ';', o:nl(1))" />
        <!-- Pointer Setter
        <xsl:value-of select="concat(o:t(2), o:cpp_func_def('void', '', o:cpp_base_set_func($variable_name), concat($variable_type, '* val')), ';', o:nl(1))" /> -->
        
        <!-- Copy Getter -->
        <xsl:value-of select="concat(o:t(2), o:cpp_func_def($variable_type, '', o:cpp_base_get_func($variable_name), ''), ';', o:nl(1))" />
        <!-- Inplace Getter -->
        <xsl:value-of select="concat(o:t(2), o:cpp_func_def($variable_ptr_type, '', $variable_name, ''), ';', o:nl(1))" />
    </xsl:function>
    ...
    <xsl:function name="o:define_attribute_functions">
        ...
        <xsl:value-of select="concat('{', o:nl(1))" />
        <xsl:value-of select="concat(o:t(1), 'auto a = GetAttribute(', o:dblquote_wrap($variable_name), ').lock();', o:nl(1))" />
        <xsl:value-of select="concat(o:t(1), 'if(a){', o:nl(1))" />
        <xsl:value-of select="concat(o:t(2), 'a', o:fp(), 'set_', $attr_type, '(val);', o:nl(1))" />
        <xsl:value-of select="concat(o:t(1),'}', o:nl(1))" />
        <xsl:value-of select="concat('}', o:nl(1))" />
        <xsl:value-of select="o:nl(1)" />
        ...
        <xsl:value-of select="concat('{', o:nl(1))" />
        <xsl:value-of select="concat(o:t(1), 'if(val){', o:nl(1))" />
        <xsl:value-of select="concat(o:t(2), o:cpp_base_set_func($variable_name), '(*val);', o:nl(1))" />
        <xsl:value-of select="concat(o:t(1),'}', o:nl(1))" />
        <xsl:value-of select="concat('};', o:nl(1))" />
        <xsl:value-of select="o:nl(1)" /> -->
        ...
        <xsl:value-of select="concat('{', o:nl(1))" />
        <xsl:value-of select="concat(o:t(1), $cpp_type, ' val;', o:nl(1))" />
        <xsl:value-of select="concat(o:t(1), 'auto a = GetAttribute(', o:dblquote_wrap($variable_name), ').lock();', o:nl(1))" />
        <xsl:value-of select="concat(o:t(1), 'if(a){', o:nl(1))" />
        <xsl:value-of select="concat(o:t(2), 'val = a', o:fp(), 'get_', $attr_type, '();', o:nl(1))" />
        <xsl:value-of select="concat(o:t(1),'}', o:nl(1))" />
        <xsl:value-of select="concat(o:t(1), 'return val;', o:nl(1))" />
        <xsl:value-of select="concat('}', o:nl(1))" />
        <xsl:value-of select="o:nl(1)" />
        ...
        <xsl:value-of select="concat('{', o:nl(1))" />
        <xsl:value-of select="concat(o:t(1), 'auto a = GetAttribute(', o:dblquote_wrap($variable_name), ').lock();', o:nl(1))" />
        <xsl:value-of select="concat(o:t(1), 'return a', o:fp(), $attr_type, '();', o:nl(1))" />
        <xsl:value-of select="concat('}', o:nl(1))" />
        <xsl:value-of select="o:nl(1)" />
    </xsl:function>
    ...
    <xsl:function name="o:declare_enum_functions">
        ...
        <xsl:value-of select="concat(o:t($tab), 'public:', o:nl(1))" />
        <!-- Copy Setter -->
        <xsl:value-of select="concat(o:t($tab + 1), o:cpp_func_def('void', '', o:cpp_base_set_func($enum_name), concat($enum_type, ' val')), ';', o:nl(1))" />
        <!-- Pointer Setter -->
        <xsl:value-of select="concat(o:t($tab + 1), o:cpp_func_def('void', '', o:cpp_base_set_func($enum_name), 'int val'), ';', o:nl(1))" />
        
        <!-- Copy Getter -->
        <xsl:value-of select="concat(o:t($tab + 1), o:cpp_func_def($enum_type, '', o:cpp_base_get_func($enum_name), ''), ' const;', o:nl(1))" />
        <!-- Inplace Getter -->
        <xsl:value-of select="concat(o:t($tab + 1), o:cpp_func_def($enum_ptr_type, '', $enum_name, ''), ';', o:nl(1))" />

        <!-- Private Declarations -->
        <xsl:value-of select="concat(o:t($tab), 'private:', o:nl(1))" />
        <!-- Variable -->
        <xsl:value-of select="concat(o:t($tab + 1), o:cpp_var_decl($enum_type, concat($enum_name, '_')), ';', o:nl(1))" />
    </xsl:function>
    ...
    <xsl:function name="o:define_enum_functions">
        ...
        <xsl:value-of select="concat('{', o:nl(1))" />
        <xsl:value-of select="concat(o:t(1), 'this', o:fp(), $variable_var, ' = val;', o:nl(1))" />
        <xsl:value-of select="concat('};', o:nl(1))" />
        <xsl:value-of select="o:nl(1)" />
        ...
        <xsl:value-of select="concat('{', o:nl(1))" />
        <xsl:value-of select="concat(o:t(1), 'this', o:fp(), $variable_var, ' = static_cast', o:angle_wrap($enum_type), '(val);', o:nl(1))" />
        <xsl:value-of select="concat('};', o:nl(1))" />
        <xsl:value-of select="o:nl(1)" />

        <!-- Copy Getter -->
        <xsl:value-of select="o:cpp_func_def($enum_type, $class_name, o:cpp_base_get_func($enum_name), '')" />
        <xsl:value-of select="concat(' const {', o:nl(1))" />
        <xsl:value-of select="concat(o:t(1), 'return this', o:fp(), $variable_var, ';', o:nl(1))" />
        <xsl:value-of select="concat('};', o:nl(1))" />
        <xsl:value-of select="o:nl(1)" />
        ...
        <xsl:value-of select="concat('{', o:nl(1))" />
        <xsl:value-of select="concat(o:t(1), 'return this', o:fp(), $enum_name, '_;', o:nl(1))" />
        <xsl:value-of select="concat('};', o:nl(1))" />
        <xsl:value-of select="o:nl(1)" />
    </xsl:function>
    ...
    <xsl:function name="o:define_variable_functions">
        ...
        <xsl:value-of select="concat('{', o:nl(1))" />
        <xsl:value-of select="concat(o:t(1), 'if(val){', o:nl(1))" />
        <xsl:value-of select="concat(o:t(2), 'this', o:fp(), $variable_var, ' = *val;', o:nl(1))" />
        <xsl:value-of select="concat(o:t(1),'}', o:nl(1))" />
        <xsl:value-of select="concat('};', o:nl(1))" />
        <xsl:value-of select="o:nl(1)" />
        ...
        <xsl:value-of select="o:cpp_func_def($variable_ptr_type, $class_name, $variable_name, '')" />
        <xsl:value-of select="concat('{', o:nl(1))" />
        <xsl:value-of select="concat(o:t(1), 'return this', o:fp(), $variable_name, '_;', o:nl(1))" />
        <xsl:value-of select="concat('};', o:nl(1))" />
        <xsl:value-of select="o:nl(1)" />
    </xsl:function>
...
<xsl:function name="cdit:get_parent_node">
    ...
    <xsl:function name="o:xml_wrap">
        ...
        <xsl:choose>
            <xsl:when test="contains($flat_val, o:nl(1))">
                <!-- Multiline -->
                <xsl:value-of select="concat(o:t($tab), o:angle_wrap($open_bracket), o:nl(1))" />
                <xsl:value-of select="$flat_val" />
                <xsl:value-of select="concat(o:t($tab), o:angle_wrap_end($key_name), o:nl(1))" />
            </xsl:when>
            <xsl:otherwise>
                <!-- Single line -->
                <xsl:value-of select="concat(o:t($tab), o:angle_wrap($open_bracket), $flat_val, o:angle_wrap_end($key_name), o:nl(1))" />
            </xsl:otherwise>
        </xsl:choose>
    </xsl:function>
    ...
    <xsl:function name="cdit:get_top_level_cmake_cmake">
        <xsl:value-of select="concat('cmake_minimum_required (VERSION 3.1)', o:nl(1))" />
        <xsl:value-of select="o:nl(1)" />
        <xsl:value-of select="concat('set(CMAKE_CXX_STANDARD 11)', o:nl(1))" />
        <xsl:value-of select="concat('set(CMAKE_CXX_STANDARD_REQUIRED ON)', o:nl(1))" />
        <xsl:value-of select="concat('set(CMAKE_CXX_EXTENSIONS OFF)', o:nl(1))" />
        <xsl:value-of select="o:nl(1)" />
        <xsl:value-of select="concat('set(CMAKE_POSITION_INDEPENDENT_CODE ON)', o:nl(1))" />
        <xsl:value-of select="o:nl(1)" />
        <xsl:value-of select="concat('set(CMAKE_MACOSX_RPATH ON)', o:nl(1))" />
        <xsl:value-of select="concat('set(CMAKE_BUILD_WITH_INSTALL_RPATH ON)', o:nl(1))" />
        <xsl:value-of select="concat('set(CMAKE_INSTALL_RPATH ', o:dblquote_wrap(concat(o:at(), 'loader_path;', o:and(), 'ORIGIN')), ')', o:nl(1))" />
        <xsl:value-of select="o:nl(1)" />

        <xsl:variable name="lib_dir" select="concat(o:cmake_var_wrap('CMAKE_CURRENT_SOURCE_DIR'), '/lib')" />

        <xsl:value-of select="concat('set(CMAKE_RUNTIME_OUTPUT_DIRECTORY ', o:dblquote_wrap($lib_dir), ')', o:nl(1))" />
        <xsl:value-of select="concat('set(CMAKE_LIBRARY_OUTPUT_DIRECTORY ', o:dblquote_wrap($lib_dir), ')', o:nl(1))" />
        <xsl:value-of select="concat('set(CMAKE_ARCHIVE_OUTPUT_DIRECTORY ', o:dblquote_wrap($lib_dir), ')', o:nl(1))" />

        <xsl:value-of select="concat('if(MSVC)', o:nl(1))" />
        <xsl:value-of select="concat(o:t(1), 'set(CMAKE_RUNTIME_OUTPUT_DIRECTORY_DEBUG ', o:dblquote_wrap($lib_dir), ')', o:nl(1))" />
        <xsl:value-of select="concat(o:t(1), 'set(CMAKE_RUNTIME_OUTPUT_DIRECTORY_RELEASE ', o:dblquote_wrap($lib_dir), ')', o:nl(1))" />
        <xsl:value-of select="concat(o:t(1), 'set(CMAKE_LIBRARY_OUTPUT_DIRECTORY_DEBUG ', o:dblquote_wrap($lib_dir), ')', o:nl(1))" />
        <xsl:value-of select="concat(o:t(1), 'set(CMAKE_LIBRARY_OUTPUT_DIRECTORY_RELEASE ', o:dblquote_wrap($lib_dir), ')', o:nl(1))" />
        <xsl:value-of select="concat(o:t(1), 'set(CMAKE_ARCHIVE_OUTPUT_DIRECTORY_DEBUG ', o:dblquote_wrap($lib_dir), ')', o:nl(1))" />
        <xsl:value-of select="concat(o:t(1), 'set(CMAKE_ARCHIVE_OUTPUT_DIRECTORY_RELEASE ', o:dblquote_wrap($lib_dir), ')', o:nl(1))" />
        <xsl:value-of select="concat('endif(MSVC)', o:nl(1))" />
