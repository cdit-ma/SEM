...
<xsl:stylesheet version="2.0" 
    ...
    <!-- Input Parameter-->
    <xsl:param name="components"></xsl:param>
    <xsl:param name="preview"></xsl:param>

    <xsl:variable name="components_to_generate" select="tokenize(normalize-space(lower-case($components)), ',')" />
    <xsl:variable name="generate_all_code" select="lower-case($preview) != 'true'" />

    <!-- Get all of the Aggregates -->
    <xsl:variable name="component_impls" as="element()*" select="cdit:get_entities_of_kind(., 'ComponentImpl')" />
    ...
    <xsl:template match="/">
        ...
        <xsl:for-each select="$component_impls">
            ...
            <xsl:if test="index-of($components_to_generate, $component_label_lc) or count($components_to_generate) = 0">
                ...
                <!-- If we are in Preview Mode only generate out Impl -->
                <xsl:if test="$generate_all_code=true()">
        ...
        <xsl:if test="$generate_all_code=true()">
            ...
            <xsl:result-document href="{o:xsl_wrap_file($components_cmake)}">
                <xsl:value-of select="cdit:get_component_folder_cmake($component_impls, $components_to_generate)" />
            </xsl:result-document>
        </xsl:if>
    </xsl:template>
</xsl:stylesheet>
