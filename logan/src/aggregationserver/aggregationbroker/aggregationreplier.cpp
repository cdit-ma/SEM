...
public:
    ...
    const std::vector<std::string>& getColumns() const { return cols_; }
    const std::vector<std::string>& getValues() const { return vals_; }
};
    ...
    for(const auto& pair : exp_name_id_pairs) {
        ...
        for(const auto& row : results) {
            ...
            try {
                ...
                std::cerr << "An exception occurred while querying experiment run end time, likely "
                             "due to null: "
                          << e.what() << "\n";
            }
        }
    ...
    const AggServer::ExperimentStateRequest& message)
{
    ...
    try {
        ...
        std::cerr << "An exception occured while populating ExperimentStateResponse with "
                     "ExperimentRunID="
                  << run_id << std::endl;
        throw;
    }
...
{
    std::unique_ptr<AggServer::PortLifecycleResponse> response =
        std::unique_ptr<AggServer::PortLifecycleResponse>(new AggServer::PortLifecycleResponse());
...
{
    std::unique_ptr<AggServer::WorkloadResponse> response =
        std::unique_ptr<AggServer::WorkloadResponse>(new AggServer::WorkloadResponse());
...
{
    std::unique_ptr<AggServer::MarkerResponse> response =
        std::unique_ptr<AggServer::MarkerResponse>(new AggServer::MarkerResponse());
                                                       ...
                                                       int experiment_run_id)
{
    ...
    try {
        const auto& results = database_->GetValues("ComponentInstance",
                                                   {"ComponentInstanceID", "Name", "ComponentID",
                                                    "Path", "GraphmlID"},
        ...
        std::cerr << "An exception occured while populating ExperimentStateResponse Container with "
                     "ContainerID="
                  << container_id << std::endl;
        throw;
    }
}
