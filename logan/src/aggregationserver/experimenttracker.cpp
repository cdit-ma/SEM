...
int ExperimentTracker::RegisterExperimentRun(
    ...
    try {
        ...
        if (experiment_id == -1) {
            throw std::runtime_error(std::string("Unable to register experiment with name ")+experiment_name);
        }
...
void ExperimentTracker::ShutdownExperimentRun(
    const std::string& experiment_name,
    const google::protobuf::Timestamp& timestamp
) {
