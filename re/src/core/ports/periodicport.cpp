...
// REVIEW (Mitch): Actually investigate leaky bucket model, apply if suitable (or give modeller
...
: ::SubscriberPort<base_type>(component, name, callback, "periodic"){
    //Force set the kind
    // REVIEW (Mitch): We should probably specify that we're calling Port's implementation of
    //  SetKind (ADL evil).
