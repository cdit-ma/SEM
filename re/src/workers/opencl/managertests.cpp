...
enum Result { UNKNOWN, PASS, FAIL, SKIPPED };
...
void testLoadBalancer(Manager& manager);

void testBufferReadWrite(Manager& manager, Device& device);
void testKernelPassthrough(Manager& manager, Device& device);
...
void testWorkerRectMult(OpenCL_Worker& worker,
                        unsigned int rowsA,
                        unsigned int colsA,
                        unsigned int rowsB,
                        unsigned int colsB,
...
int main(int argc, char** argv)
{
    ...
    OpenCL_Worker test_worker(test_component, "ManagerTestWorker");
    ...
    Manager* manager;
    try {
        manager = Manager::GetReferenceByPlatformID(0, &test_worker);
        res = PASS;
    } catch(const std::exception& e) {
        std::cerr << "Exception when obtaining manager reference: " << e.what() << std::endl;
        res = FAIL;
    }
    recordTest(res, "Retrieved first available Manager without an exception being thrown");
    ...
    recordTest(res, "Attempt to retreive reference to first available Manager did not return null");
    if(manager->IsValid()) {
        res = PASS;
    } else {
        res = FAIL;
    }
    recordTest(res, "Reference to first available Manager reports itself as being valid");
    if(res == FAIL) {
        return 1;
    }
...
void testLoadBalancer(Manager& manager)
{
    ...
    LoadBalancer load_balancer(device_ids);
...
void testBufferReadWrite(Manager& manager, Device& device)
{
    ...
    OpenCL_Worker test_worker(test_component, "TestBufferReadWriteWorker");
    ...
    /*if (in_data.size() != out_data.size()) {
            ...
            for (int i=0; i<in_data.size(); i++) {
                    if (in_data[i] != out_data[i]) {
                            std::cout << "Mismatch at element " << i << ": expected " << in_data[i] << ", got " <<
    out_data[i] << std::endl; vectors_match = false;
                    }
            }
    }*/
    ...
    if(out_data.size() != out_data2.size()) {
        ...
        for(int i = 0; i < in_data.size(); i++) {
            if(out_data[i] != out_data2[i]) {
                std::cout << "Mismatch at element " << i << ": expected " << out_data[i] << ", got " << out_data2[i]
                          << std::endl;
                vectors_match = false;
            }
        }
...
void testKernelPassthrough(Manager& manager, Device& device)
{
    ...
    OpenCL_Worker passthroughWorker(passthroughComponent, "PassthroughWorker");
    ...
    /*if(*/ device.LoadKernelsFromSource(filenames); //) {
    /*    res = PASS;
    } else {
        res = FAIL;
    }
...
void testWorkerCreateBufferOfType(OpenCL_Worker& worker, std::string type_name, std::vector<T> vec_4)
{
    ...
    if(vec_4_out.size() != vec_4.size())
        contents_equal = false;
    for(int i = 0; i < vec_4.size(); i++) {
        if(vec_4_out[i] != vec_4[i])
            contents_equal = false;
    }
    ...
    recordTest(res, "Create buffer for length 4 " + type_name
                        + " vector returned data equal to that with which it was constructed");
    auto vec_5(vec_4);
    vec_5.push_back(0);
...
void testWorkerCreateBuffer(OpenCL_Worker& worker)
{
    ...
    // testWorkerCreateBufferOfType<bool>(worker, "bool", {true, false, false, true});	// Should throw static
    // assertion! testWorkerCreateBufferOfType<std::string>(worker, "string", {"This", "is", "a", "test"});	//
    // Should throw static assertion!
}

void individualParallelRun(OpenCL_Worker& worker, int threads, cl_long ops)
{
    Result res = UNKNOWN;
    // std::cout << "Calling OpenCL_Worker function RunParallel with "<<threads<<" thread, "<<ops<<" operation... " <<
    // std::endl;
    ...
    recordTest(res, "Execute RunParallel with " + std::to_string(threads) + " thread, " + std::to_string(ops)
                        + " operations");
...
void testWorkerSquareMult(OpenCL_Worker& worker, unsigned int length, bool expect_failure)
{
    ...
    recordTest(res, "MatrixMult kernel did report success for running with size " + std::to_string(length) + "x"
                        + std::to_string(length));

    if(calculation_finished) {
        res = checkMultiplication(matA.data(), matB.data(), matC.data(), length, length, length);
    }
    recordTest(res, "Correct values for multiplication of zeroed square matrix of size " + std::to_string(length) + "x"
                        + std::to_string(length));

    for(unsigned int index = 0; index < elements; index++)
        matA[index] = (float)index;
    for(unsigned int index = 0; index < elements; index++)
        matB[index] = (float)index;
    for(unsigned int index = 0; index < elements; index++)
        matC[index] = std::numeric_limits<float>::signaling_NaN();
    ...
    srand(RANDSEED);
    for(unsigned int index = 0; index < elements; index++)
        matA[index] = (float)(rand() % 1000000000) / 100 - 1000000;
    for(unsigned int index = 0; index < elements; index++)
        matB[index] = (float)(rand() % 1000000000) / 100 - 1000000;
    for(unsigned int index = 0; index < elements; index++)
        matC[index] = std::numeric_limits<float>::signaling_NaN();
...
void testWorkerRectMult(OpenCL_Worker& worker,
                        ...
                        bool expect_failure)
{
    printInfo("Rect " + std::to_string(rowsA) + "x" + std::to_string(colsA) + " by " + std::to_string(rowsB) + "x"
              + std::to_string(colsB));
    ...
    for(unsigned int index = 0; index < lenA; index++)
        matA[index] = 0.0;
    for(unsigned int index = 0; index < lenB; index++)
        matB[index] = 0.0;
    // for (unsigned int index=0; index<lenC; index++) matC[index]=-1.0;
    for(unsigned int index = 0; index < lenC; index++)
        matC[index] = std::numeric_limits<float>::signaling_NaN();
    ...
    recordTest(res, "MatrixMult kernel did report success for running with size " + std::to_string(rowsA) + "x"
                        + std::to_string(colsA) + " by " + std::to_string(rowsB) + "x" + std::to_string(colsB));

    if(calculation_finished) {
        res = checkMultiplication(matA.data(), matB.data(), matC.data(), rowsA, colsA, colsB);
    }
    recordTest(res, "Correct values for multiplication of zeroed matrix of size " + std::to_string(rowsA) + "x"
                        + std::to_string(colsA) + " by " + std::to_string(rowsB) + "x" + std::to_string(colsB));
    ...
    recordTest(res, "Correct values for multiplication of element-wise-incrementing matrix of size "
                        + std::to_string(rowsA) + "x" + std::to_string(colsA) + " by " + std::to_string(rowsB) + "x"
                        + std::to_string(colsB));
    ...
    recordTest(res, "Correct values for multiplication of pseudo-random matrix of size " + std::to_string(rowsA) + "x"
                        + std::to_string(colsA) + " by " + std::to_string(rowsB) + "x" + std::to_string(colsB));
}
        ...
        for(unsigned int row = 0; row < m; row++) {
            ...
            if(!compare_float(accum, matC[col + row * n], EPS)) {
                if(m * n * k > 100)
                    return FAIL;
                // if (verbose) {
                std::cout << "At pos [" << row << ',' << col << "] expected: " << accum
                          << ", result: " << matC[col + row * n] << std::endl;
...
void testWorkerKMeans(OpenCL_Worker& worker)
{
    ...
    std::vector<float> point_vec = {10, 4, 5, 7, 19, 3, 6, 5, 2, 11, 0, -3, 4, 9, 1, 0, 13, 5, 4, 9, 7, 7, 2, 2};
    std::vector<float> center_vec = {0, 10, 0, 0, 10, 1, 1, 7};
    ...
    std::vector<int> expected_classes = {1, 1, 0, 0, 1, 0};
    ...
    center_vec.clear();
    center_vec = {250, 350, 900, 400, 450, 750, 200, 300, 650, 300, 750, 900};
    ...
    for(int i = 0; i < 2000; i++) {
        if(class_vec[i] != 1) {
            ...
            std::cout << point_vec[i * 4] << ',' << point_vec[i * 4 + 1] << ',' << point_vec[i * 4 + 2] << ','
                      << point_vec[i * 4 + 3] << ',' << std::endl;
            res = FAIL;
            break;
        }
    }
