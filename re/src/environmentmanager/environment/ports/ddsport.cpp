...
namespace re::EnvironmentManager::dds {
...
void Port::FillPortPb(NodeManager::Port& port_pb)
{
    ...
    if(kind == Kind::Publisher) {
        ...
        if(!blackbox_domain_ids.empty()) {
            if(blackbox_domain_ids.size() > 1) {
                std::cerr << "* Experiment[" << GetExperiment().GetName()
                          << "]: Has multiple blackbox domain ids connected to Port: '" << GetId()
                          << "'" << std::endl;
            }
            std::cerr << "* Experiment[" << GetExperiment().GetName()
        ...
        if(domain_ids.size() > 1) {
            std::cerr << "* Experiment[" << GetExperiment().GetName()
                      << "]: Has multiple domains connected to Port: '" << GetId() << "'"
                      << std::endl;
        }
        if(domain_ids.empty()) {
            std::cerr << "* Experiment[" << GetExperiment().GetName() << "]: Port: '" << GetId()
                      << "' Has no connected ports, defaulting to user set domain_id." << std::endl;
        }

        // Set topic name and domain id. If none found based on connected ports, use value set
        // on port.
        int domain_id = !domain_ids.empty() ? *domain_ids.begin() : GetDomainId();
        NodeManager::SetIntegerAttribute(attrs, "domain_id", domain_id);
